...
Mean steps per day: **`r fmt1(stepsByDay$steps.sum, mean)`**

Median steps per day: **`r fmt1(stepsByDay$steps.sum, median)`**
...
Interval of maximum number of steps: **`r interval`**

Maximum average steps for an interval: **`r format(max(stepsByTime$steps.mean), big.mark=",", trim=TRUE)`**

Interval time for maximum average steps for an interval: **`r intervalTime`**
...
Mean steps per day (with imputed values): **`r fmt1(stepsByDay2$steps.sum, mean)`**

Median steps per day (with imputed values): **`r fmt1(stepsByDay2$steps.sum, median)`**

#### 4c\. Do these values differ from the estimates from the first part of the assignment?

Yes. The mean and median total steps per day calculated with NA values was **`r fmt1(stepsByDay$steps.sum, mean)`** and **`r fmt1(stepsByDay$steps.sum, median)`**, respectively. The mean and median total steps per day calculated with imputed values was **`r fmt1(stepsByDay2$steps.sum, mean)`** and **`r fmt1(stepsByDay2$steps.sum, median)`**, respectively.
...
```{r fig4-timeseries-panel, echo=TRUE}
